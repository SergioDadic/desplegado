...
export default function ModalVariosPedidos() {
  ...
  const handleFileChange = (event) => {
    if (event && event.target && event.target.files.length > 0) {
        ...
        const archivosSeleccionados = event.target.files[0];
        setFile(archivosSeleccionados);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        ...
        const response = await axiosCliente().post("back/api/v1/simulacion/diaria/pedidoTiempoReal",formData,
            {
                headers: {
